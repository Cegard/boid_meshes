class Boid {
  ...
  PShape wholeModel = createShape(GROUP);

  Boid(PVector inPos) {
    wholeModel.disableStyle();
  ...
  void setStructures(){
    ...
    vertices[0] = new PVector(3 * sc, 0, 0);
    vertices[1] = new PVector(-3 * sc, 2 * sc, 0);
    vertices[2] = new PVector(-3 * sc, -2 * sc, 0);
    vertices[3] = new PVector(-3 * sc, 0, 2 * sc);
    
    int[] triangle_1 = {0, 1, 2};
    int[] triangle_2 = {0, 1, 3};
    int[] triangle_3 = {0, 3, 2};
    int[] triangle_4 = {3, 1, 2};
    
    triangles[0] = triangle_1;
    triangles[1] = triangle_2;
    triangles[2] = triangle_3;
    triangles[3] = triangle_4;
  }
  ...
  void buildModel(){
    ...
    for (int[] triangleVertices : triangles){
      PShape singleFace = createShape();
      singleFace.beginShape(TRIANGLE);
      
      for (int vertexIndex : triangleVertices){
        singleFace.vertex(vertices[vertexIndex].x,
               vertices[vertexIndex].y,
               vertices[vertexIndex].z);
      }
      
      singleFace.endShape();
      wholeModel.addChild(singleFace);
    }
  }
